...
Plotter q;
    ...
    MatrixXd Debugger(3,3);
    VectorXd AccelVec(3);
    VectorXd GravityVec(3);
    MatrixXd TransformerMat(3,3); //robots in disguise
...
void setup(){
  ...
  q.AddTimeGraph("Raw Gyro X", 15000, "gx", gyrX);
  q.AddTimeGraph("Raw Gyro Y", 15000, "gy", gyrY);
  q.AddTimeGraph("Raw Gyro Z", 15000, "gz", gyrZ);
  TheState.setZero(); //clean garbage
      ...
      if (serialOption!=0) {
        ...
        switch(serialOption) {
          ...
          case 8:
            ...
            Serial.print('\t');
            //Serial.print('\n');
            Serial.print(TheState(6));
            Serial.print('\t');
            Serial.print(TheState(7));
            Serial.print('\t');
            Serial.println(TheState(8));        
          break;
...
void getSensor(double& ax, double& ay, double& dt){
    ...
    cphi = cosdWhy(-TheState(11));
    sphi = sindWhy(-TheState(11));
    ctheta = cosdWhy(-TheState(10));
    stheta = sindWhy(-TheState(10));
    cpsi = cosdWhy(-TheState(9));
    spsi = sindWhy(-TheState(9));
/*
    TransformerMat << cpsi*cphi - ctheta*sphi*spsi,  cpsi*sphi + ctheta*cphi*spsi,  stheta*spsi,
                      -spsi*cphi - ctheta*sphi*cpsi, -spsi*sphi + ctheta*cphi*cpsi, stheta*cpsi,
                      sphi*stheta,                   -cphi*stheta,                  ctheta;*/

    TransformerMat << ctheta*cphi,                   ctheta*sphi,                  -stheta,
...
/*    TransformerMat <<  cosd(TheState(10))*cosd(TheState(9)), cosd(TheState(10))*sind(TheState(9)), -sind(TheState(10)),
                    -cosd(TheState(11))*sind(TheState(9))+sind(TheState(11))*sind(TheState(10))*cosd(TheState(9)), cosd(TheState(11))*cosd(TheState(9))+sind(TheState(11))*sind(TheState(10))*sind(TheState(9)), sind(TheState(11))*cosd(TheState(10)),
                    sind(TheState(11))*sind(TheState(9))+cosd(TheState(11))*sind(TheState(10))*cosd(TheState(9)), -sind(TheState(11))*cosd(TheState(9))+cosd(TheState(11))*sind(TheState(10))*sind(TheState(9)), cosd(TheState(11))*cosd(TheState(10));*/
  ...
  //  AccelVec = TransformerMat * AccelVec;
    GravityVec = TransformerMat.transpose() * GravityVec;
    
    ax = AccelVec(0) * g - GravityVec(0);
    ay = AccelVec(1) * g - GravityVec(1);
    az = AccelVec(2) * g - GravityVec(2);
...
//Who knows why including Trigd doesn't work...
double sindWhy(double x){
  return sin(x*M_PI/180.0);
}

double cosdWhy(double x){
  return cos(x*M_PI/180.0);
}
 
double tandWhy(double x){
